...
pipeline {
    ...
    stages {
        ...
        stage('Dev Sonarqube SAST') {
            ...
            steps { 
                withSonarQubeEnv('devopsb39-SonarQube-DEV'){
                     ...
                     -Dsonar.host.url=http://sonar.akshaik8s.xyz:9000/"
                }
        ...
        stage('Dev docker build') {
            ...
            agent { label 'DEV' }
            steps { 
                sh "sudo docker build -t akshai183/devopsb39spring:$BUILD_NUMBER ."
            }
        }
        stage('Dev Trivy Scan') {
            ...
            steps {
                        ...
                        sh """sudo trivy image akshai183/devopsb39spring:$BUILD_NUMBER --security-checks vuln --exit-code 0 --severity CRITICAL --timeout 15m --format template --template \"@html.tpl\" --output reports/dev_trivy_report.html  """
                        sh 'aws s3 cp reports/dev_trivy_report.html s3://akshaik8s.xyz/dev_trivy_report.html'
                    }
        ...
        stage('Publish Trivy Report') {
            ...
            agent { label 'MASTER' }
            steps {
                        sh 'aws s3 cp s3://akshaik8s.xyz/dev_trivy_report.html dev_trivy_report.html'
        ...
        stage('Dev Deploy Docker Image') {
            ...
            steps { 
                ...
                sh "sudo docker run --rm -dit --name springbootapp -p 8080:8080 akshai183/devopsb39spring:$BUILD_NUMBER"
            }
        }
        ...
        stage ('Dev DAST') {
          ...
          agent { label 'DEV' }  
          steps {
             sh 'sudo docker run -t owasp/zap2docker-stable zap-baseline.py -t http://dev.akshaik8s.xyz:8080 || true'
            }
        }
//-----------------------------PRODUCTION---------------
        ...
        stage('PROD docker build') {
            ...
            agent { label 'PROD' }
            steps { 
                sh "sudo docker build -t akshai183/devopsb39springprod:$BUILD_NUMBER ."
            }
        }
        stage('Prod Trivy Scan') {
            ...
            steps {
                        ...
                        sh """sudo trivy image akshai183/devopsb39springprod:$BUILD_NUMBER --security-checks vuln --exit-code 0 --severity CRITICAL --timeout 15m --format template --template \"@html.tpl\" --output reports/prod_trivy_report.html  """
                        sh 'aws s3 cp reports/prod_trivy_report.html s3://akshaik8s.xyz/prod_trivy_report.html'
                    }
                }
        stage('Publish Prod Trivy Report') {
            ...
            agent { label 'MASTER' }
            steps {
                        sh 'aws s3 cp s3://akshaik8s.xyz/prod_trivy_report.html prod_trivy_report.html'
        ...
        stage('PROD Deploy Docker Image') {
            ...
            steps { 
                sh "sudo docker stop springbootapp || sudo docker ps"
                sh "sudo docker run --rm -dit --name springbootapp -p 8080:8080 akshai183/devopsb39springprod:$BUILD_NUMBER"
            }
        }
        stage('PROD Validate Deployment') {
            ...
            agent { label 'PROD' }
            steps { 
                sh "sleep 30 && curl http://prod.akshaik8s.xyz:8080 || exit 1"
            }
        }
        stage ('PROD DAST') {
            when {
          ...
          agent { label 'PROD' }  
          steps {
             sh 'sudo docker run -t owasp/zap2docker-stable zap-baseline.py -t http://prod.akshaik8s.xyz:8080 || true'
            }
        }
    }
    post {
